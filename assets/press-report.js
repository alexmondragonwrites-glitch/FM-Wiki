(()=>{
  ...
  function render(){
    ...
    if(!root)return;
    if(!r){
      root.innerHTML='<section class="empty-state"><div><h1>Bericht nicht gefunden</h1><p>Die Pressedaten konnten nicht geladen werden oder der Link ist nicht mehr gültig.</p><a class="button button-blue" href="index.html">Zur Startseite</a></div></section>';
      return;
    }
  ...
  const ready=window.FM_DOMAIN_READY?.press;
  if(ready&&typeof ready.then==='function')ready.then(render).catch(error=>{
    console.error('Pressedaten konnten nicht geladen werden.',error);
    render();
  });
  else render();
})();
