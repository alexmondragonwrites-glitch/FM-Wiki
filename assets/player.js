...
const referenceDate='2040-11-02';
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const fmtRating=v=>Number(v).toFixed(2).replace('.',',');
...
function renderAttributes(attributes){
  ...
  return `<section class="content-card"><p class="kicker dark">ATTRIBUTPROFIL</p><h2>Was sein Spiel wirklich trägt</h2><div class="attribute-groups">${Object.entries(attributes).map(([group,values])=>`<article class="attribute-group"><h3>${esc(group)}</h3>${Object.entries(values).map(([label,value])=>`<div class="attribute-row"><b>${esc(label)}</b><span class="attribute-value ${valueClass(value)}">${value}</span></div>`).join('')}</article>`).join('')}</div><span class="source-chip">Quelle: Spielerprofil vom 02.11.2040</span></section>`;
}
...
function renderDeepAnalysis(items){
  if(!items?.length)return '';
  return `<section class="content-card"><p class="kicker dark">TIEFENANALYSE</p><h2>Warum Ramsey mehr als seine Tore ist</h2><div class="deep-analysis-grid">${items.map(x=>`<article class="deep-analysis-card"><h3>${esc(x.title)}</h3><p>${esc(x.text)}</p></article>`).join('')}</div></section>`;
}
...
if(!p){
  ...
  const identityFields=[
    ...
    ['Medienbeschreibung',p.mediaDescription],
    ['Starker Fuß',p.preferredFoot],
    ['Nationalmannschaft',p.international],
    ['U21',p.youthInternational],
    ['Vertrag bis',p.contractUntil],
    ['Gehalt',p.wage],
    ['Kaderstatus',p.status]
  ].filter(([,v])=>v);
  const recordStrip=(p.xg||p.playerOfMatch)?`<div class="profile-record-strip"><div><span>Tore 2040</span><strong>${p.goals??'–'}</strong></div><div><span>xG 2040</span><strong>${p.xg?.toFixed(2).replace('.',',')??'–'}</strong></div><div><span>Über xG</span><strong>${p.xg!=null&&p.goals!=null?`+${(p.goals-p.xg).toFixed(2).replace('.',',')}`:'–'}</strong></div><div><span>Spieler des Spiels</span><strong>${p.playerOfMatch??'–'}</strong></div></div>`:'';
  ...
  root.innerHTML=`
    <section class="profile-hero">
      ...
      <div class="profile-title">
        <div>
          <p class="kicker">${esc(p.nation)} · ${esc(p.positionCode)} · ${esc(p.systemRole&&p.systemRole!=='-'?p.systemRole:p.role)}</p>
          <h1>${esc(p.name)}</h1>
          <p>${esc(p.summary)}</p>
        </div>
        <div class="profile-monogram">${esc(p.name.split(' ').map(x=>x[0]).join('').slice(0,2))}</div>
      </div>
    </section>
    <div class="profile-shell">
      <section class="profile-facts">
        <div><span>Geburtsdatum</span><strong>${utils.formatBirthDate(p.birthDate)}</strong></div>
        <div><span>Alter am 02.11.2040</span><strong>${calculatedAge}${p.birthDate?'':'*'}</strong></div>
      ...
      <div class="profile-content">
        <div>
          ...
          <section class="content-card">
            <p class="kicker dark">GESAMTURTEIL</p>
            <h2>${esc(p.verdict)}</h2>
            <div class="analysis-callout">${esc(p.analysis||p.summary)}</div>
            <p>${esc(p.summary)}</p>
            ${recordStrip}
            ${p.birthDate?'':`<p class="source-note">* Das genaue Geburtsdatum ist noch nicht erfasst. Das Alter stammt aus dem aktuellen Export.</p>`}
          </section>
          ...
          <section class="content-card">
            <p class="kicker dark">SAISON 2040</p>
            <h2>Leistungsprofil</h2>
            <div class="metric-grid">
              <div><span>Einsätze</span><strong>${esc(p.appearances)}</strong></div>
              <div><span>Tore</span><strong>${p.goals??'–'}</strong></div>
              <div><span>Vorlagen</span><strong>${p.assists??'–'}</strong></div>
              <div><span>Ø-Note</span><strong>${p.avgRating?fmtRating(p.avgRating):'–'}</strong></div>
              <div><span>Kondition</span><strong>${esc(p.condition||'–')}</strong></div>
              <div><span>Moral</span><strong>${esc(p.morale)}</strong></div>
            </div>
          </section>
          ${renderSeasonSplits(p.seasonSplits)}
          <section class="content-card">
            <p class="kicker dark">TAKTISCHE EINORDNUNG</p>
            <h2>${esc(p.systemRole&&p.systemRole!=='-'?p.systemRole:p.role)}</h2>
            <p><b>Beste Rolle laut Export:</b> ${esc(p.bestRole)}</p>
            <p><b>Positionen:</b> ${esc(p.positionCode)}</p>
            <p>Die Systemrolle beschreibt den aktuellen Einsatz im 4-3-3. Die beste Rolle ist die Football-Manager-Empfehlung und muss nicht identisch mit der taktischen Aufgabe sein.</p>
          </section>
          ${renderAttributes(p.attributes)}
          ${renderTraits(p.traits)}
          ${renderDeepAnalysis(p.deepAnalysis)}
          ${renderCareer(p.careerSeasons,p.careerTotals)}
          ${renderNarrative(p.careerNarrative)}
          ${renderHonours(p.honours)}
          <section class="content-card">
            <p class="kicker dark">SAISONHISTORIE</p>
            <h2>Im Kaderarchiv verfolgen</h2>
            <p>Geburtsdatum und Saisonstichtag werden genutzt, um das Alter in jedem archivierten Kader automatisch korrekt zu berechnen.</p>
            <div class="hero-actions"><a class="button button-blue" href="kaderarchiv.html?year=2040">Kader 2040</a><a class="button button-blue" href="kaderarchiv.html?year=2041">Kader 2041</a></div>
          </section>
        </div>
        <aside>
          ...
          <section class="content-card"><p class="kicker dark">VERGLEICHSGRUPPE</p><h2>${esc(p.group)}</h2><div class="peer-list">${peers.map(x=>`<a href="${x.page}"><span>${esc(x.name)}</span><b>${x.avgRating?fmtRating(x.avgRating):'–'}</b></a>`).join('')}</div></section>
        </aside>
      </div>
    </div>`;
}