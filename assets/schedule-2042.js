(()=>{
  const fixtures=(window.FM_FIXTURES||[])
    .filter(item=>String(item[0]).startsWith('2042-'))
  ...
  const opponentLabel=value=>value||'Noch offen';
  ...
  const referenceDate=String(window.FM_CONFIG?.referenceDate||'2041-12-31');
  const next=fixtures.find(f=>!f.result&&f.date>=referenceDate)||fixtures.find(f=>!f.result);
  const nextBox=document.querySelector('#next2042');
  if(next&&nextBox){
    const date=dateParts(next.date);
    nextBox.innerHTML=`<div><p class="kicker">NÄCHSTES SPIEL</p><h2>${esc(opponentLabel(next.opponent))}</h2><p>${esc(date.long)} · ${esc(next.time)} · ${esc(next.venue)}</p><small>${esc(next.competition)}</small></div><strong>vs.</strong>`;
  }
  ...
  function render(){
    ...
    const filtered=fixtures.filter(f=>(!query||`${opponentLabel(f.opponent)} ${f.competition} ${f.venue} ${f.result||''}`.toLowerCase().includes(query))&&(competitionValue==='all'||f.competition===competitionValue)&&(venueValue==='all'||f.venue===venueValue));
        ...
        <div class="fixture-list">
          ${items.map(f=>{
            ...
            return `<article class="fixture-row ${played?'played':'upcoming'}">
              ...
              ${timeOrResult}
              <div class="fixture-opponent"><b>${esc(opponentLabel(f.opponent))}</b><small>${opponentMeta}</small></div>
