(()=>{
  ...
  const teamCode=name=>({Irland:'IRL',Spanien:'ESP',Frankreich:'FRA',Niederlande:'NED',Belgien:'BEL',Schottland:'SCO',Tschechien:'CZE',Kroatien:'CRO',Zypern:'CYP',Aserbaidschan:'AZE'}[name]||String(name||'').slice(0,3).toUpperCase());
  ...
  const eventIcon=type=>type==='goal'?'⚽':type==='yellow'?'🟨':type==='red'?'🟥':type==='injury'?'✚':'•';
  const eventHtml=(m.events||[]).map(e=>`<li class="event-${e.team==='Irland'?'ireland':'opponent'}"><span>${esc(e.minute)}'</span><b>${eventIcon(e.type)} ${esc(e.player)}</b><small>${esc(e.team)}${e.assist?` · Vorlage ${esc(e.assist)}`:''}</small></li>`).join('');
  ...
  const tournament=window.FM_NATIONS_LEAGUE_2041;
  const reportSection=document.querySelector('.national-match-report');
  if(tournament&&reportSection){
    reportSection.insertAdjacentHTML('beforeend',`<div class="content-card"><p class="kicker dark">FINAL FOUR · ABSCHLUSS</p><h2>${esc(tournament.winner)} gewinnt die Nations League 2041</h2><p>Finale: ${esc(tournament.final.home)} ${tournament.final.homeScore}:${tournament.final.awayScore} ${esc(tournament.final.away)} · Spiel um Platz 3: ${esc(tournament.thirdPlace.home)} ${tournament.thirdPlace.homeScore}:${tournament.thirdPlace.awayScore} ${esc(tournament.thirdPlace.away)}.</p><div class="match-stats"><div class="match-stat-row"><strong>1.</strong><span>${esc(tournament.winner)}</span><strong>Titel</strong></div><div class="match-stat-row"><strong>2.</strong><span>${esc(tournament.runnerUp)}</span><strong>Finale</strong></div><div class="match-stat-row"><strong>3.</strong><span>${esc(tournament.third)}</span><strong>Platz 3</strong></div><div class="match-stat-row"><strong>4.</strong><span>${esc(tournament.fourth)}</span><strong>Platz 4</strong></div></div></div>`);
  }
})();